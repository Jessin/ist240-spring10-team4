...
namespace WindowsFormsApplication1
{
    public partial class Form1 : Form
    {
        ...
        private void TestButton_Click(object sender, EventArgs e)
        {
            ...
            if (lightrainB == true)
            {
                actualTest.WeatherCode = 1;
            }
            if (lightsnowB == true)
            {
                actualTest.WeatherCode = 2;
            ...
            {
                actualTest.Light = "ON";
                actualTest.WeatherCode = 3;
            }
            if (heavysnowB == true)
            {
                actualTest.Light = "ON";
                actualTest.WeatherCode = 4;
            }
            if (overheatB == true)
            {
                actualTest.ErrorCode = 1;

            }
            if (washerfluidB == true)
            {
                actualTest.ErrorCode = 2;
            }
            if (tirepressureB == true)
            {
                int returnValue = library_dll.Entities.utility.RandomNumber(4, 7);
                actualTest.ErrorCode = returnValue;
            }
            if (oilB == true)
            {
                actualTest.ErrorCode = 3;

            }
            if (freeB == true)
            {
                actualTest.TraffiCode = 1;
            }
            if (lightTB == true)
            {
                actualTest.TraffiCode = 2;

            }
            if (mediumB == true)
            {
                actualTest.TraffiCode = 3;
            }
            if (heavyB == true)
            {
                actualTest.TraffiCode = 4;
            }
            int directionValue = library_dll.Entities.utility.RandomNumber(0, 7);
            actualTest.DirectionCode = directionValue;

            XmlSerializer serializer = new XmlSerializer(typeof(library_dll.Entities.TestingSystem));
            TextWriter texterWriter = new StreamWriter("tester.xml");
